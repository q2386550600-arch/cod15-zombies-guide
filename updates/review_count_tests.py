...
Usage: python updates/review_count_tests.py --reviewed 67 --drafts 115 --extra-ids 558453,558329,...
...
if extra:
    ...
    assert s.count(marker)==1;s=s.replace(marker,block+marker)
...
if extra:
    # Add a separate exact check rather than mutating the legacy twelve-page list.

...
extra='''    # Exercise the exact Buried gallery complaint on the installed APK.
    ...
    page.tap('#imageLanguage');page.wait('document.querySelector("#fullAction").textContent.includes("断头台")','Chinese explanation did not render')
    snap('08-buried-Chinese-explanation');page.tap('#imageLanguage')
...
pre='''    old=Path('old/COD_Zombies_Guide_v0.9.apk')
    ...
    prior.tap('#openNotes');prior.tap('#freeNotes')
    adb('shell','input','text','upgrade-keep-0245')
    prior.wait('GuideDebug.getState().notes.alpha==="upgrade-keep-0245"','Native text input did not save the v0.9 note')
    prior.close();adb('shell','input','keyevent','KEYCODE_HOME');time.sleep(3)

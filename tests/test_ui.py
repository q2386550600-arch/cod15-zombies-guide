...
errors=[];bad=[];checks=[]
def image_ready(page, selector='#stepPhoto'):
 # Poll from Python: do not ask wait_for_function to eval a string under CSP.
 deadline=time.monotonic()+20
 while time.monotonic()<deadline:
  if page.locator(selector).evaluate('(im) => im.complete && im.naturalWidth > 0'):
   return
  time.sleep(.05)
 raise AssertionError('Image failed to decode: '+str(page.locator(selector).get_attribute('src')))
...
with sync_playwright() as p:
 ...
 page.click('#openNotes');page.locator('#freeNotes').fill('恢复测试：本局密码 0245');page.click('#closeModal');assert not page.locator('#modal').is_visible()
 page.reload();page.click('#start');assert page.locator('#stepTitle').inner_text()=='启动 Rushmore'
 page.click('#openNotes');assert page.locator('#freeNotes').input_value()=='恢复测试：本局密码 0245';page.click('#closeModal');assert not page.locator('#modal').is_visible();checks.append('Progress and notes persist after reload')
 for game,keys in page.evaluate('Object.entries(window.ZOMBIE_GAMES).map(([k,v])=>[k,v.maps])'):
  ...
  for key in keys:
   print('Testing map:',key,flush=True)
 ...
 page.click('#closeModal');page.click('#start');page.click('#openIndex');page.locator('#modalBody .previewItem').nth(3).click();image_ready(page);page.screenshot(path=str(OUT/'tv-step.png'))
